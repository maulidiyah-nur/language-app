...
const QuestionComponent = (props: IQuestionProps) => {
  ...
  if (isMissingWord && !answer) {
    return (
      <View style={[styles.container, styles.missingContainer]}>
        <Text color={ComponentConstant.Colors.PRIMARY}>{trans?.value}</Text>
      </View>
    );
  }
  if (isMissingWord && answer) {
    ...
    if (!isError && !isValid) {
      return (
        <Button type="white" size="small">
          <Text strong color={ComponentConstant.Colors.PRIMARY}>
            {aTrans?.value}
          </Text>
        </Button>
      );
      ...
      return (
        <Button type="valid" size="small">
          <Text strong color={ComponentConstant.Colors.LIGHT}>
            {aTrans?.value}
          </Text>
        </Button>
      );
